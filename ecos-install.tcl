...
# restart using a Tcl shell \
    exec sh -c 'for tclshell in tclsh tclsh83 cygtclsh80 ; do \
        ...
        echo "ecos-install.tcl: cannot find Tcl shell" ; exit 1' "$0" "$@"
...
#===============================================================================
# TODO list:
# Incorporate tarball and uncompressed sizes into ecos-install.db
...
# installer really!
# Checksums on downloaded data in ecos-install.db. Then we can also check for
...
# ----------------------------------------------------------------------------
# ecos-install.tcl requires at least version 8.0 of Tcl, since it makes use of
...
#        variable mirror $script_base
        ...
        variable metadata_url { "http://sources.redhat.com/ecos/ecos-install.db" \
                                "ftp://sources.redhat.com/pub/ecos/ecos-install.db" \
                                "http://mirrors.rcn.net/pub/sourceware/ecos/ecos-install.db" \
                            }
...
proc ecos_install::argument_help { } {
        ...
        puts "Usage: ecos-install.tcl \[OPTION\] \[INSTALLPATH\] \[ toolchain-prefix \]..."
        puts "  e.g. ecos-install.tcl /opt/ecos arm-elf i386-elf"
        puts ""
        puts "Options:"
...
proc ecos_install::retrieve_url { url quiet outputfilename } {
        ...
        for {set retries 3} {$retries > 0} {incr retries -1} {
			...
			# The above is what we're meant to do, but a cygwin TCL bug means we have to do:
			set wget_env_hack "http_proxy=$::env(http_proxy) ftp_proxy=$::env(ftp_proxy)"
                ...
                if { $outputfilename != "" } {
                        set wget_saved_dir [pwd]
                        cd [file dirname $outputfilename]
                        set wget_extra_args "$wget_extra_args -O [file tail $outputfilename]"
                }

                set wget_output ""
                set wget_passive_ftp ""
                if { [string equal -nocase -length 4 "ftp:" $url] == 1} {
		        set wget_passive_ftp "--passive-ftp"
		}

                set wget_fd [ open "|/bin/sh -c \"$wget_env_hack wget -c -T 30 -t 3 $wget_extra_args $wget_passive_ftp $url\" |& cat -" "r" ]
                if { $wget_saved_dir != "" } {
                    cd $wget_saved_dir
                }
                ...
                if { [catch "close $wget_fd" code] } {
                        ...
                        if { $quiet != 0 } {
                                ...
                                if { $retries > 1 && $ecos_install::proxy != "" } {
...
proc ecos_install::mk_install_dir { } {
        if { $ecos_install::install_dir == "" } {
                ...
                if { ([file exists /opt/ecos] && [file writable /opt/ecos]) || \
                        ...
                        set defaultdir "/opt/ecos"
                } else {
                        set defaultdir "$::env(HOME)/ecos"
                }
                ecos_install::report "---------------------------------------------------------"
                set status -1
                while { $status < 0 } {
...
proc ecos_install::get_ecos_install_data { } {
        ...
        if { $ecos_install::mirror != "" && $ecos_install::metadata_url_supplied == 0 } {
            ...
            set ecos_install::metadata_url [ list [ ecos_install::file_or_url_join $ecos_install::mirror "ecos-install.db" ] ]
        }
        set tmpfile "/tmp/ecos-install.db.[pid]"
...
#        }
        ...
        if { [file exists ecosenv.sh] } {
                ecos_install::report "Updating [ file nativename [ file join $idir ecosenv.sh ]]"
                set scr_fd [open "ecosenv.sh" "r" ]
                set newscr_fd [open "ecosenv.sh.new" "w" ]
                ...
                close $scr_fd
                close $newscr_fd
                make_file_executable "ecosenv.sh.new"
                file rename -force -- ecosenv.sh.new ecosenv.sh
        } else {
                ecos_install::report "Generating [file nativename [ file join $idir ecosenv.sh]]"
                set newscr_fd [open "ecosenv.sh" "w"]
                puts $newscr_fd "# Warning! This is a generated file used by the eCos installer."
                puts $newscr_fd "# If you edit this file, you may break future upgrades using the installer.\n"
                if { $ecos_install::ecos_repo_suffix != "" } {
                        puts $newscr_fd "ECOS_REPOSITORY=[file join $idir $ecos_install::ecos_repo_suffix] ; export ECOS_REPOSITORY\n"
                        puts $newscr_fd "# eCos paths - do not modify this line, it is used by the installer"
                        foreach ecospath $ecos_install::ecos_path_suffixes {
                                puts $newscr_fd "PATH=[file join $idir $ecospath]:\$PATH ; export PATH"
                        }
                        puts $newscr_fd "# End eCos paths - do not modify this line, it is used by the installer\n"
                }
                foreach toolpath $ecos_install::gnutool_path_suffixes {
                        puts $newscr_fd "PATH=[file join $idir $toolpath]:\$PATH ; export PATH"
                }
                close $newscr_fd
                make_file_executable "ecosenv.sh"
        }
        if { [file exists ecosenv.csh] } {
                ecos_install::report "Updating [ file join $idir ecosenv.csh ]"
                set scr_fd [open "ecosenv.csh" "r" ]
                set newscr_fd [open "ecosenv.csh.new" "w" ]
                set toolpath_list [list]
                foreach toolpath $ecos_install::gnutool_path_suffixes {
                ...
                close $scr_fd
                close $newscr_fd
                make_file_executable "ecosenv.csh.new"
                file rename -force -- ecosenv.csh.new ecosenv.csh
        } else {
                ecos_install::report "Generating [file nativename [ file join $idir ecosenv.csh]]"
                set newscr_fd [open "ecosenv.csh" "w"]
                puts $newscr_fd "# Warning! This is a generated file used by the eCos installer."
                puts $newscr_fd "# If you edit this file, you may break future upgrades using the installer.\n"
                if { $ecos_install::ecos_repo_suffix != "" } {
                        puts $newscr_fd "setenv ECOS_REPOSITORY [file join $idir $ecos_install::ecos_repo_suffix]\n"
                ...
                close $newscr_fd
                make_file_executable "ecosenv.csh"
        }
        ecos_install::report "---------------------------------------------------------"
        ecos_install::report "In future, to establish the correct environment for eCos,"
        ecos_install::report "run one of the following commands:"
        ecos_install::report "   . [file join $idir ecosenv.sh] \t\t(for sh/bash users); or"
        ecos_install::report "   source [file join $idir ecosenv.csh] \t(for csh/tcsh users)\n"
...
if { ! [info exists ecos_install_not_standalone] } {
        ...
        if { [ catch {
                ...
                ecos_install::report "eCos installer v2.0 starting..."
